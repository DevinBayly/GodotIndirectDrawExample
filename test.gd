...
func side_compute(buffer,pos_buffer):
...
func _ready() -> void:
	...
	if true: #Indirect args
		...
		pos_buffer = rd.storage_buffer_create(buf.size(),buf)
		print("pos buf",pos_buffer)
		side_compute(indirect_args,pos_buffer)
